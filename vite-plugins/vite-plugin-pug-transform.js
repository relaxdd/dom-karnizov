...
/**
 * Некоторая доработка плагина vite-plugin-pug-transformer@1.0.3
 *
 * @param pugOptions
 * @param pugLocals
 * @return {*|{transformIndexHtml: {transform(*, {filename: *}): *, enforce: string}, handleHotUpdate({file: *, server: *}): ([]|undefined), name: string}|*[]}
 */
  ...
  return {
    ...
    transformIndexHtml: {
      ...
      transform(html, { filename }) {
        return html.replace(/<template.*?data-type="pug".*?(\/>|<\/template>)/g, (matchedString) => {
